...
class Api::V1::ItemsController < ApplicationController
    ...
    def fullMatch
        begin
            practice_id = params[:practice_id]
            if practice_id
                @respo = ActiveRecord::Base.connection.execute("SELECT distinct d.id as def_id,d.item_name as def_item_name, d.custom_id as def_custom_id, d.categories as def_categories, d.param_type as def_param_type, d.param_value as def_param_value, d.name as def_name, d.remaining_name as def_remaining_name, d.second_name as def_second_name,f.id as p_id,f.item_name as p_item_name, f.custom_id as p_custom_id, f.categories as p_categories, f.param_type as p_param_type, f.param_value as p_param_value, f.name as p_name, f.remaining_name as p_remaining_name, f.second_name as p_second_name, case when m.id is not null then 'mapped' else '' end as status,m.id as mapped_item  FROM public.formatted_items as f inner join public.formatted_items d on d.name = f.name and d.custom_practice_id != #{practice_id} and d.second_name = f.second_name and d.param_type = f.param_type and d.param_value = f.param_value and d.categories = f.categories left outer join mapped_items m on f.id = m.formatted_item_id where f.custom_practice_id =  #{practice_id}  and m.id is null ")
                # and f.id not in (select * from (SELECT distinct f.id FROM public.default_items as d inner join public.formatted_items f on d.item_name = f.item_name where f.custom_practice_id = #{practice_id}) as a)
                render_json(200, @respo.count.to_s+" record/s found!",@respo)
            else
                render_json(400, "Practice Id is required!",[])
            end
    ...
    def lowMatch
        begin
            ...
            if practice_id
                ...
                # @respo = FormattedItem.joins("inner join public.formatted_items d on d.name = formatted_items.name and d.custom_practice_id != #{practice_id} and d.param_type = formatted_items.param_type and d.param_value = formatted_items.param_value and d.categories = formatted_items.categories left outer join mapped_items m on formatted_items.id = m.formatted_item_id where m.id is null and formatted_items.custom_practice_id = #{practice_id} and formatted_items.param_type != '' and formatted_items.id not in (select * from (SELECT distinct formatted_items.id FROM public.default_items as d inner join public.formatted_items f on d.item_name = formatted_items.item_name where formatted_items.custom_practice_id = #{practice_id} union all SELECT distinct formatted_items.id FROM public.default_items as d inner join public.formatted_items f on d.name = formatted_items.name and d.second_name = formatted_items.second_name and d.param_type = formatted_items.param_type and d.param_value = formatted_items.param_value and d.categories = formatted_items.categories where formatted_items.custom_practice_id =  #{practice_id}) as a)")

                @respo = ActiveRecord::Base.connection.execute("SELECT distinct d.id as def_id,f.id as p_id,d.item_name as def_item_name, d.custom_id as def_custom_id, d.categories as def_categories, d.param_type as def_param_type, d.param_value as def_param_value, d.name as def_name, d.remaining_name as def_remaining_name, d.second_name as def_second_name,f.item_name as p_item_name, f.custom_id as p_custom_id, f.categories as p_categories, f.param_type as p_param_type, f.param_value as p_param_value, f.name as p_name, f.remaining_name as p_remaining_name, f.second_name as p_second_name, case when m.id is not null then 'mapped' else '' end as status,m.id as mapped_item  FROM public.formatted_items as f inner join public.formatted_items d on d.name = f.name and d.custom_practice_id != #{practice_id} and d.param_type = f.param_type and d.param_value = f.param_value and d.categories = f.categories left outer join mapped_items m on f.id = m.formatted_item_id where f.custom_practice_id = #{practice_id} and f.param_type != ''")
     ...
     def remaining
        begin
            ...
            if practice_id
                ...
                @respo = ActiveRecord::Base.connection.execute("select distinct f.*, case when m.id is not null then 'mapped' else '' end as status,m.id as mapped_item   from formatted_items f  left outer join mapped_items m on f.id = m.formatted_item_id  where m.id is null and f.custom_practice_id =  #{practice_id}")
        ...
        begin
            if type
                if data.nil? || data.length == 0
                    ...
                    if delete_previous 
                        ActiveRecord::Base.connection.execute("truncate default_items")
